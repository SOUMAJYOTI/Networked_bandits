...
def model_gs_matching_manytomany(u_b, u_l, c, q, obj_util, lambda_1, lambda_2, LogToConsole=True, TimeLimit=60):
    model = Model()
    model.params.LogToConsole = LogToConsole
    model.params.TimeLimit = TimeLimit  # seconds
    x = {}
    ...
    for b_idx in c:
        for l_idx in q:
            ...
            for l_idx_2 in q:
                if l_idx != l_idx_2:
                    if u_b[b_idx][l_idx] < u_b[b_idx][l_idx_2]:
                        constr_obj_3 += (q[l_idx_2] * x[b_idx][l_idx_2])
...
def model_gs_matching_with_fairness(u_b, u_l, c, q, obj_util, fair_constr, lambda_1, lambda_2, lambda_3, LogToConsole=True, TimeLimit=60):
    ...
    model.setObjective(lambda_1 * quicksum(obj_util[b_idx][l_idx] * x[b_idx][l_idx] for l_idx in q for b_idx in c) - \
                       lambda_2 * quicksum(w[b_idx][l_idx] for l_idx in q for b_idx in c) - lambda_3 * fair_constr, GRB.MAXIMIZE)
    model.optimize()
    if model.status != 2:
        print("Optimal Solution not found !!!")
        return -1, -1, -1
